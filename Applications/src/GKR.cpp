...
void parse_mul_tree(ifstream &circuit_in, long long int *in, int N);
...
struct proof prove_multree(vector<F> data, vector<F> randomness,int input_size){
    ...
    parse_mul_tree(circuit_in,in, input_size);
    c = DAG_to_layered();
    c.subsetInit();
...
void parse_mul_tree(ifstream &circuit_in, long long int *in,int leaves_size){
    vector<int> input(leaves_size);
    int counter = 0;
