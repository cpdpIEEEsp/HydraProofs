...
int bit_decomposition_size(struct proof P){
	...
	if(P.type == RANGE_PROOF_OPT){
		for(int i = 0; i < P.Partial_sums.size(); i++){
			for(int j = 0; j < P.Partial_sums[i].size(); j++){
				size++;
			}
		}
		size += P.individual_sums.size();
	}
	return size*32;
}
...
void verify_bit_decomposition(struct proof Pr){
	...
	if(Pr.type == RANGE_PROOF_OPT){
		...
		for(int i = 0; i < Pr.K; i++){
			for(int j = i+1; j < Pr.K; j++){
				sum2 += F(2)*Pr.r[i]*Pr.r[j]*Pr.Partial_sums[i][j-i-1];
			}
		}
		sum2 = F(0) - sum2;
		vector<F> mul(Pr.K,F(0));
		for(int i = 0; i < Pr.K; i++){
		...
		for(int i = 0; i < Pr.K; i++){
			sum2 += mul[i]*Pr.individual_sums[i]*Pr.r[i];
		}
	}
	...
	if(Pr.type == RANGE_PROOF_OPT){
		...
		if(sum != Pr.vr[2]*(neg-Pr.vr[2])*Pr.vr[3]){
...
float get_proof_size(vector<proof> proofs){
	...
	for(int i = 0; i < proofs.size(); i++){
		if(proofs[i].type == MATMUL_PROOF){
			size += m2m_size(proofs[i]);
		}
		else if(proofs[i].type == RANGE_PROOF || proofs[i].type == RANGE_PROOF_OPT || proofs[i].type == RANGE_PROOF_LOOKUP){
			size += bit_decomposition_size(proofs[i]);
		}
		else if(proofs[i].type == GKR_PROOF){
			size += gkr_proof_size(proofs[i]);
		}
		else{
			//printf("Proof not found\n");
		}
