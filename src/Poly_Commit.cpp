...
void compute_proofs(vector<F> &poly,Comm &commitment,vector<vector<G1>> &proof_tree, int level){
	if(poly.size() == 1){
	...
	compute_proofs(poly1,commitment,proof_tree,level+1);
	compute_proofs(poly2,commitment,proof_tree,level+1);
}
...
void hyperproofs_openall(vector<F> poly,Comm &commitment){
	...
	compute_proofs(poly,commitment,proof_tree,0);	
}
