...
void parse_smallness_range_proof(int size);

void parse_hadamard_product(ifstream &circuit_in, long long int *in, int N);
void parse_check_max(ifstream &circuit_in, long long int *in, int N, int M);
void parse_sqrt_matrix(ifstream &circuit_in, long long int *in, int N);
void parse_pardon_div(ifstream &circuit_in, long long int *in, int N, int M);
void parse_check_pardon_div(ifstream &circuit_in, long long int *in, int N, int M);
void parse_sqrt_aux(ifstream &circuit_in, long long int *in, int N);
void parse_diagonal_extraction(ifstream &circuit_in, long long int *in, int N);
void parse_log(ifstream &circuit_in, long long int *in, int N);
void parse_inv_relu(ifstream &circuit_in, long long int *in, int N);
void parse_range_proof(ifstream &circuit_in, long long int *in, int N);
...
struct proof range_proof(vector<F> data, vector<F> randomness, int N){
    ...
    parse_range_proof(circuit_in,in,N);
    c = DAG_to_layered();
...
struct proof prove_inv_relu(vector<F> data, vector<F> randomness, int N){
    ...
    ifstream circuit_in;
    in_circuit_dag.clear();
    parse_inv_relu(circuit_in,in,N);
    c = DAG_to_layered();
...
struct proof extract_diagonal(vector<F> data, vector<F> randomness, int N){
    ...
    ifstream circuit_in;
    in_circuit_dag.clear();
    parse_diagonal_extraction(circuit_in,in,N);
    c = DAG_to_layered();
...
struct proof sqrt_aux(vector<F> data, vector<F> randomness, int N){
    ...
    ifstream circuit_in;
    in_circuit_dag.clear();
    parse_sqrt_aux(circuit_in,in,N);
    c = DAG_to_layered();
...
struct proof sqrt_matrix(vector<F> data, vector<F> randomness, int N){
    ...
    ifstream circuit_in;
    in_circuit_dag.clear();
    parse_sqrt_matrix(circuit_in,in,N);
    c = DAG_to_layered();
...
struct proof check_max(vector<F> data, vector<F> randomness, int N, int M){
    ...
    ifstream circuit_in;
    in_circuit_dag.clear();
    parse_check_max(circuit_in,in,N,M);
    c = DAG_to_layered();
...
struct proof check_pardon_div(vector<F> data, vector<F> randomness, int N, int M){
    ...
    ifstream circuit_in;
    in_circuit_dag.clear();
    parse_check_pardon_div(circuit_in,in,N,M);
    c = DAG_to_layered();
...
struct proof pardon_div(vector<F> data, vector<F> randomness, int N, int M){
    ...
    ifstream circuit_in;
    in_circuit_dag.clear();
    parse_pardon_div(circuit_in,in,N,M);
    c = DAG_to_layered();
...
struct proof hadamard_product(vector<F> data, vector<F> randomness, int N){
    ...
    ifstream circuit_in;
    in_circuit_dag.clear();
    parse_hadamard_product(circuit_in,in,N);
    c = DAG_to_layered();
...
struct proof prove_smallness(vector<F> data, vector<F> randomness,int size){
    ...
    ifstream circuit_in;
    in_circuit_dag.clear();
    parse_smallness_range_proof(size);
    c = DAG_to_layered();
...
void parse_verification_circuit(vector<vector<int>> transcript){
    ...
    for(int i =0 ; i < transcript.size(); i++){
        if(transcript[i][0] == 1){
            gkr_proofs.push_back(read_gkr(transcript[i],counter,1));
        }else if(transcript[i][0] == 2){
            range_proofs.push_back(read_range_proof(transcript[i],counter,1));
        }
        else{
            m2m_proofs.push_back(read_M2M(transcript[i],counter,1));            
        }
    }
    buildInput(counter, 0);
    int one =(counter);
...
void parse_smallness_range_proof(int size){
    int counter = 0;
    vector<int> x1,x2,x3;
    vector<int> mx1,mx2,mx3;
    for(int i = 0; i <size; i++){
        buildInput(counter, 0);
        x1.push_back(counter);
        counter++;
    }
    for(int i = 0; i < size; i++){
        buildInput(counter, 0);
        x2.push_back(counter);
        counter++;
    }    
    for(int i = 0; i < size; i++){
        buildInput(counter, 0);
        x3.push_back(counter);
        counter++;
    }
    for(int i = 0; i < size; i++){
        mul(x1[i],x1[i],counter);
        mx1.push_back(counter);
        mul(x2[i],x2[i],counter);
        mx2.push_back(counter);
        mul(x3[i],x3[i],counter);
        mx3.push_back(counter);
    }
    for(int i = 0; i < size; i++){
    //    add(mx1[i],mx2[i],counter);
    //    add(counter-1,mx3[i],counter);
    }
}
...
void compute_log(int num,vector<int> parameters1,vector<int> parameters2, int &counter){
    vector<int> powers(8);
    vector<int> partial_log(8);
    powers[0] = num;
    mul(powers[0],powers[0],counter);
    powers[1] = counter-1;
    mul(powers[0],powers[1],counter);
    powers[2] = counter-1;
    mul(powers[1],powers[1],counter);
    powers[3] = counter-1;
    mul(powers[0],powers[3],counter);
    powers[4] = counter-1;
    mul(powers[1],powers[3],counter);
    powers[5] = counter-1;
    mul(powers[2],powers[3],counter);
    powers[6] = counter-1;
    mul(powers[3],powers[3],counter);
    powers[7] = counter-1;

    for(int i = 0; i < 8; i++){
        mul(powers[i],parameters1[i],counter);
        mul(counter-1,parameters2[i],counter);
        partial_log[i] = counter-1;
    }
    
    sub(partial_log[0],partial_log[1],counter);
    
    sub(partial_log[2],partial_log[3],counter);
    sub(partial_log[4],partial_log[5],counter);
    int pos1 = counter-1;
    sub(partial_log[6],partial_log[7],counter);
    int pos2 = counter-1;
    
    add(counter-4,counter-3,counter);
    add(counter-1,pos1,counter);
    add(counter-1,pos2,counter);

}

void parse_range_proof(ifstream &circuit_in, long long int *in, int N){
    vector<int> bits(8*N),neg_bits(8*N);
    vector<int> nums(N);
    vector<int> powers(8);
    int counter = 0;
    for(int i = 0; i < 8*N; i++){
    ...
    vector<int> num_bits(8);
    for(int i = 0; i < N; i++){
        for(int j = 0; j < 8; j++){
            num_bits[j] = bits[i*8 + j];
        }
        ip(num_bits,powers,counter);
    }
    for(int i = 0; i < N; i++){
        mul(neg_bits[i],bits[i],counter);
    }
}


void parse_inv_relu(ifstream &circuit_in, long long int *in, int N){
    vector<int> msb1(N),msb2(N),input(N);
    int one;
    int counter = 0;
    for(int i = 0; i < N; i++){
        buildInput(counter,0);
    ...
    for(int i = 0; i < N; i++){
        mul(msb1[i],input[i],counter);
        add(counter-1,msb2[i],counter);
    }
...
void parse_log(ifstream &circuit_in, long long int *in, int N){
    vector<int> in_left(N),in_right(N);
    vector<int> parameters1(N),parameters2(N);
    int counter = 0;
    for(int i = 0; i < N; i++){
        buildInput(counter,0);
        in_left[i] = counter;
    ...
    for(int i = 0; i < 8; i++){
        buildInput(counter,0);
        parameters1[i] = counter;
        counter++;        
    }
    for(int i = 0; i < 8; i++){
        buildInput(counter,0);
        parameters2[i] = counter;
        counter++;        
    }

    for(int i = 0; i < N; i++){
        compute_log(in_left[i],parameters1,parameters2,counter);
    }
    for(int i = 0; i < N; i++){
        compute_log(in_right[i],parameters1,parameters2,counter);
    }
}
...
void parse_sqrt_aux(ifstream &circuit_in, long long int *in, int N){
    ...
    for(int i = 0; i < N; i++){
        add(arr[i],one,counter);
        mul(counter-1,counter-1,counter);
    }
    for(int i = 0; i < N; i++){
        mul(arr[i],arr[i],counter);
        mul(counter-1,one,counter);
    }
...
void parse_check_max(ifstream &circuit_in, long long int *in, int N, int M){
    ...
    for(int i = 0; i < M; i++){
        for(int j = 0; j < N; j++){
            buildInput(counter,0);
            input[i].push_back(counter);
            counter++;
        }
    }
    for(int i = 0; i < M; i++){
        mul_tree(input[i],counter);
    }
}

void parse_check_pardon_div(ifstream &circuit_in, long long int *in, int N, int M){
    vector<int> max(N);
    vector<int> shifted_max(N);
    ...
    for(int i = 0; i < N; i++){
        for(int j = i+1; j < N; j++){
            mul(max[j],input[i*N + j],counter);
            sub(shifted_max[i],remainders[i*N + j],counter);
...
void parse_pardon_div(ifstream &circuit_in, long long int *in, int N, int M){
    ...
    for(int i = 0; i < N; i++){
        ...
        for(int j = i+1; j < N; j++){
            mul(max[j],input[i*N + j],counter);
            sub(shifted_max[i],counter-1,counter);
        }
    }
